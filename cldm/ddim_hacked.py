...
class DDIMSampler(object):
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      dynamic_threshold=None, constrastive=True):
    ...
    def rotation_sample(self,
        ...
        size = (batch_size, C, H, W)
        print(f'Data shape for DDIM sampling is {size}, eta {eta}')
        samples, intermediates = self.rotation_ddim_sampling(conditioning, size,
                                                             callback=callback,
                                                             img_callback=img_callback,
                                                             quantize_denoised=quantize_x0,
                                                             mask=mask, x0=x0,
    ...
    @torch.no_grad()
    def rotation_ddim_sampling(self, cond, shape,
                               x_T=None, ddim_use_original_steps=False,
                               callback=None, timesteps=None, quantize_denoised=False,
                               mask=None, x0=None, img_callback=None, log_every_t=100,
                               temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                               unconditional_guidance_scale=1., unconditional_conditioning=None, dynamic_threshold=None,
                               ucg_schedule=None, rotation_step=4):
        device = self.model.betas.device
        b = shape[0]
        if x_T is None:
            img = torch.randn(shape, device=device)
        ...
        width = img.shape[-1]
        iterator = tqdm(time_range, desc='DDIM Sampler', total=total_steps)
        img_step = int(width/rotation_step)
        # per_steps = total_steps // (rotation_step+1)
        per_steps = total_steps // rotation_step
        cond_step = int(img_step*8)
        cnt = 0
        for i, step in enumerate(iterator):
            index = total_steps - i - 1
            ts = torch.full((b,), step, device=device, dtype=torch.long)
            ...
            if ucg_schedule is not None:
                assert len(ucg_schedule) == len(time_range)
                unconditional_guidance_scale = ucg_schedule[i]
            if (cnt+1) % per_steps == 0:
                img = torch.roll(img, img_step, dims=-1)
                for i in range(len(cond["c_concat"])):
                    cond["c_concat"][i] = torch.roll(
                        cond["c_concat"][i], cond_step, dims=-1)
                    unconditional_conditioning["c_concat"][i] = torch.roll(
                        unconditional_conditioning["c_concat"][i], cond_step, dims=-1)
            outs = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      quantize_denoised=quantize_denoised, temperature=temperature,
                                      noise_dropout=noise_dropout, score_corrector=score_corrector,
                                      corrector_kwargs=corrector_kwargs,
        ...
        #     rotation_step = 64
        #     img_step = int(width / rotation_step)
        #     last_step = img_step + int(width % rotation_step)
        #     cond_step = int(img_step*8)
        #     cnt = 0
        #     start_step = total_steps-rotation_step
        #     for i, step in enumerate(iterator):
        #         index = total_steps - i - 1
        #         ts = torch.full((b,), step, device=device, dtype=torch.long)
        ...
        #             unconditional_guidance_scale = ucg_schedule[i]
        #         # if cnt >=start_step:
        #         # if cnt + 1 == total_steps:
        #         #     img = torch.roll(img, last_step, dims=-1)
        #         #     for i in range(len(cond["c_concat"])):
        #         #         cond["c_concat"][i] = torch.roll(
        #         #             cond["c_concat"][i], last_step*8, dims=-1)
        #         #         unconditional_conditioning["c_concat"][i] = torch.roll(
        #         #             unconditional_conditioning["c_concat"][i], cond_step, dims=-1)
        #         # else:
        #         #     img = torch.roll(img, img_step, dims=-1)
