...
import gc
def get_evaluation_results(ref_batch, sample_batch, save_result_path_list=[], batch_size=32):
    ...
    with tf.Session(config=config) as sess:
        ...
        import yaml
        for save_result_path in save_result_path_list:
            dir_name = os.path.dirname(save_result_path)
        ...
        del ref_batch, sample_batch, ref_acts, ref_stats, ref_stats_spatial, sample_acts, sample_stats, sample_stats_spatial
    ...
    return float(inception_score), float(fid), float(sfid), float(prec), float(recall)
