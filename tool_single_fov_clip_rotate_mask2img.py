...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, rotation_step):
    with torch.no_grad():
...
def parse_args():
    ...
    parser.add_argument('--img', metavar="FILE",
                        default='datasets/Structured3D/scene_03201/2D_rendering/78368/panorama/full/semantic_label.png',
                        help='img path')
    # checkpoint and log
    parser.add_argument('--prompt', type=str, default='A kitchen with windows',
                        help='prompt')
    ...
    parser.add_argument('--config-file', metavar="FILE",
                        default='./models/cldm_v15_clip_new_resize_contrastive_dcn3_rotate.yaml',
                        help='config file path')
    parser.add_argument('--resume', type=str, default='./paper_checkpoints/sota_model/perspect_resize_clip_mask_checkpoints_dcn3_constrastive_rotate_final/ControlNet-epoch=20-global_step=108464.0.bak.ckpt',
...
equ_img = Image.open(args.img).convert('L')
...
res = Image.fromarray(ips[1]).save(
    os.path.join(args.save_path, 'res_rotate.png'))
res2 = Image.fromarray(np.roll(ips[1],512,axis=1)).save('res_rotate_180.png')